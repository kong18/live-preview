  ...
  try {
    ...
    if (!isValid) {
      return res.status(403).json({
        ...
        message: 'License is invalid, expired, revoked, or bound to another machine',
      });
    }

    // Update last_seen in background (non-blocking)
    kv.hmset(`audit:${hashedKey}`, {
      last_seen: Date.now().toString(),
      last_fingerprint: fingerprint,
    }).catch((err) => console.warn('Failed to update last_seen:', err));

    // Calculate remaining time
    const expiresAt = license.expires_at ? parseInt(license.expires_at, 10) : null;
